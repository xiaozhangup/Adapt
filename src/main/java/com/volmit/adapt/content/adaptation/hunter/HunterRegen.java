...
public class HunterRegen extends SimpleAdaptation<HunterRegen.Config> {
    public HunterRegen() {
        ...
        setDescription(Localizer.component("hunter", "regen", "description"));
        setDisplayName(Localizer.component("hunter", "regen", "name"));
        ...
        if (e.getEntity() instanceof org.bukkit.entity.Player p && isAdaptableDamageCause(e) && hasAdaptation(p)) {
            ...
            if (!getConfig().useConsumable) {
                if (p.getFoodLevel() == 0) {
                    ...
                    addPotionStacks(p, PotionEffectType.REGENERATION, getLevel(p),
