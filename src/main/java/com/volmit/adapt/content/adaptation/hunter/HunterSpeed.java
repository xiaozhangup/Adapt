...
public class HunterSpeed extends SimpleAdaptation<HunterSpeed.Config> {
    public HunterSpeed() {
        ...
        setDescription(Localizer.component("hunter", "speed", "description"));
        setDisplayName(Localizer.component("hunter", "speed", "name"));
        ...
        if (e.getEntity() instanceof org.bukkit.entity.Player p && isAdaptableDamageCause(e) && hasAdaptation(p)) {
            ...
            if (!getConfig().useConsumable) {
                if (p.getFoodLevel() == 0) {
                    ...
                    addPotionStacks(p, PotionEffectType.SPEED, getLevel(p), getConfig().baseEffectbyLevel * getLevel(p),
                            getConfig().stackBuff);
                }
            } else {
                if (getConfig().consumable != null && Material.getMaterial(getConfig().consumable) != null) {
                    ...
                    if (mat != null && p.getInventory().contains(mat)) {
                        ...
                        addPotionStacks(p, PotionEffectType.SPEED, getLevel(p),
