...
public class HunterInvis extends SimpleAdaptation<HunterInvis.Config> {
    public HunterInvis() {
        ...
        setDescription(Localizer.dLocalize("hunter", "invisibility", "description"));
        setDisplayName(Localizer.dLocalize("hunter", "invisibility", "name"));
        ...
        if (e.getEntity() instanceof org.bukkit.entity.Player p && isAdaptableDamageCause(e) && hasAdaptation(p)) {
            ...
            if (!getConfig().useConsumable) {
                if (p.getFoodLevel() == 0) {
                    ...
                    addPotionStacks(p, PotionEffectType.INVISIBILITY, getLevel(p),
