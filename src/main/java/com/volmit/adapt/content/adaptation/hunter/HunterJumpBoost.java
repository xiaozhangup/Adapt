...
public class HunterJumpBoost extends SimpleAdaptation<HunterJumpBoost.Config> {
    public HunterJumpBoost() {
        ...
        setDescription(Localizer.component("hunter", "jumpboost", "description"));
        setDisplayName(Localizer.component("hunter", "jumpboost", "name"));
        ...
        if (e.getEntity() instanceof org.bukkit.entity.Player p && isAdaptableDamageCause(e) && hasAdaptation(p)) {
            ...
            if (!getConfig().useConsumable) {
                if (p.getFoodLevel() == 0) {
                    ...
                    addPotionStacks(p, PotionEffectType.JUMP_BOOST, getLevel(p), getConfig().baseEffectbyLevel * getLevel(p),
                            getConfig().stackBuff);
                }
            } else {
                if (getConfig().consumable != null && Material.getMaterial(getConfig().consumable) != null) {
                    ...
                    if (mat != null && p.getInventory().contains(mat)) {
                        ...
                        addPotionStacks(p, PotionEffectType.JUMP_BOOST, getLevel(p),
