...
public class SeaborneTurtlesMiningSpeed extends SimpleAdaptation<SeaborneTurtlesMiningSpeed.Config> {
    ...
    public SeaborneTurtlesMiningSpeed() {
        ...
        setDescription(Localizer.component("seaborn", "haste", "description"));
        setDisplayName(Localizer.component("seaborn", "haste", "name"));
        ...
        for (Player player : Adapt.instance.getAdaptServer().getAdaptPlayers()) {
            if (player.isInWater() && hasAdaptation(player)) {
                if (player.getLocation().getBlock().isLiquid()) {
                    player.addPotionEffect(new PotionEffect(PotionEffectType.HASTE, 62, 1, false, false));
                }
            }
        }
    }
