...
public class HunterLuck extends SimpleAdaptation<HunterLuck.Config> {
    public HunterLuck() {
        ...
        setDescription(Localizer.component("hunter", "luck", "description"));
        setDisplayName(Localizer.component("hunter", "luck", "name"));
    ...
    public void addStats(int level, Element v) {
        HunterText.addStats(v, level, "luck", false);
            ...
            if (!getConfig().useConsumable) {
                if (p.getFoodLevel() == 0) {
                    if (getConfig().poisonPenalty) {
                        ...
                        addPotionStacks(p, PotionEffectType.UNLUCK, getConfig().basePoisonFromLevel - getLevel(p),
                                getConfig().baseHungerDuration, getConfig().stackPoisonPenalty);
                    }
                    ...
                    addPotionStacks(p, PotionEffectType.LUCK, getLevel(p), getConfig().baseEffectbyLevel * getLevel(p),
                            getConfig().stackBuff);
                }
            } else {
                if (getConfig().consumable != null && Material.getMaterial(getConfig().consumable) != null) {
                    ...
                    if (mat != null && p.getInventory().contains(mat)) {
                        ...
                        addPotionStacks(p, PotionEffectType.LUCK, getLevel(p),
