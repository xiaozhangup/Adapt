...
package com.volmit.adapt.util.spatial.space;

import com.googlecode.concurrentlinkedhashmap.ConcurrentLinkedHashMap;
import com.googlecode.concurrentlinkedhashmap.EvictionListener;
import com.googlecode.concurrentlinkedhashmap.Weigher;
import com.volmit.adapt.util.spatial.util.Biset;
...
public class ConcurrentSpaceMap<T> implements Space<T>, Weigher<SpaceNode<T>>, EvictionListener<Long, SpaceNode<T>> {
    ...
    public ConcurrentSpaceMap(int absoluteMax, SpaceLoader<T> loader, SpaceSaver<T> saver) {
        this(Math.min(absoluteMax, Math.min((absoluteMax / 4), 128)), absoluteMax, loader, saver);
    }
    ...
    private Stream<Map.Entry<Long, SpaceNode<T>>> modifiedKeys() {
        return new HashSet<>(data.entrySet()).stream().filter(i -> i.getValue().isModified());
    }
    ...
    public void unloadAll() {
        new HashSet<>(data.entrySet()).forEach(i -> {
