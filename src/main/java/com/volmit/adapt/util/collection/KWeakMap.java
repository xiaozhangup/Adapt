...
public class KWeakMap<K, V> extends WeakHashMap<K, V> {
    ...
    public <S> KWeakMap<K, V> putValueList(K k, S... vs) {
        try {
            KWeakMap<K, KList<S>> s = (KWeakMap<K, KList<S>>) this;
     ...
     */
    public KWeakMap<K, V> put(Map<K, V> m) {
        putAll(m);
        return this;
    }
     ...
     */
    public KWeakMap<K, V> copy() {
        return new KWeakMap<K, V>(this);
    }
     ...
     */
    public KWeakMap<K, V> rewrite(Consumer3<K, V, KWeakMap<K, V>> f) {
        KWeakMap<K, V> m = copy();
     ...
     * @return the same gmap
     */
    public KWeakMap<K, V> keach(Consumer2<K, V> f) {
        for (K i : k()) {
            f.accept(i, get(i));
        }
     ...
     */
    public KWeakMap<V, K> flipFlatten() {
        KWeakMap<V, KList<K>> f = kFlip();
        KWeakMap<V, K> m = new KWeakMap<>();
     ...
     */
    public KWeakMap<V, KList<K>> kFlip() {
        KWeakMap<V, KList<K>> flipped = new KWeakMap<V, KList<K>>();
...
public KList<K> k() {
     ...
     */
    public KWeakMap<K, V> qput(K key, V value) {
        super.put(key, value);
        return this;
    }
     ...
     * @return the same map
     */
    public KWeakMap<K, V> putNonNull(K key, V value) {
